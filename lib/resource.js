...
export default class Resource {
	...
	_reducer(state = {items: {}, awaiting: []}, action) {
				...
				case actionTypes.request:
					...
					fetch(url, options)
						...
						.then((data) => {
							...
							if (data.data) {
								console.log('datums', data.data);
								const processDatum = (datum) => {
									let relationships = _.filter(_.mapValues(datum.relationships, (relationship) => {
										return relationship.data || null;
									}));
									dispatch(this.receive(datum.id, datum.attributes));
								}
								if (_.isArray(data.data)) {
									_.each(data.data, processDatum);
								} else {
									processDatum(data.data);
								}
							}
						}, (error) => {
